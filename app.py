...
def analyze_momentum_strategy(target_list, type_name="ETF"):
    ...
    if results:
        ...
        cols_order = [
            ...
            "정배열", "장기추세", "MACD-V", "ATR", "현52주신고가일", "전52주신고가일", "차이일", "현재가"
        ]
...
def get_compass_signal():
    ...
    df_display.columns = ["순수 모멘텀 점수", "조정 모멘텀 순위", "12개월 추세(절대)"]
...
def fetch_korean_etf_data(ticker, name):
    ...
    try:
        ...
        if len(df_daily) >= 60:
            ...
            if df_ind is not None:
                ...
                bb_bk_val = "O" if (c_series > df_ind['BB50_UP']).iloc[-3:].any() else "-"
                dc_bk_val = "O" if (c_series > df_ind['Donchian_High_50']).iloc[-3:].any() else "-"
    except Exception:
        pass
    ...
    try:
        ...
        return {
            ...
            'BBW': curr_bbw, '스퀴즈': "-", 'BB(50,2)돌파': "-", '돈키언(50)돌파': "-"
        }
...
def fetch_korean_etf_strategy2_data(ticker, name, start_date):
    try:
        ...
        # 당일/전일 돌파 감지 (MA50 삭제, BB상단 기준만 추출)
...
with tab_compass:
    ...
    **전략 로직:**
    ...
    2. **점수 산출:** `((12개월+6개월)/2 - 3개월) + 1개월` 수익률을 가장 최우선 기준으로 판별합니다.
    ...
    if st.button("🚀 지금 어디에 투자해야 할까? (분석 시작)", type="primary"):
        with st.spinner("최근 2년치 데이터를 분석하여 방향을 잡는 중입니다..."):
            df_display_result, position = get_compass_signal()
            
            if df_display_result is not None:
                ...
                with col2:
                    top_score = df_display_result.iloc[0]['순수 모멘텀 점수']
                    st.metric("1등 순수 모멘텀 점수", f"{top_score}점")

                st.markdown("---")
                st.markdown("#### 📊 자산별 상세 스코어 (순수 점수 높은 순)")
                
                st.dataframe(df_display_result, use_container_width=True)
                
                st.info("""
                **해석 가이드:**
                * **순수 모멘텀 점수:** 기본 점수가 높을수록 상승 모멘텀이 뛰어난 자산입니다.
                * **조정 모멘텀 순위:** 변동성을 감안(스케일링)했을 때 전체 자산 중 이 자산의 안정성 및 매력도 순위를 의미합니다.
...
with tab_korea:
    ...
    st.info("한국 주식 시장(KOSPI, KOSDAQ)의 시가총액을 그룹화하고, 모멘텀 상위 20% 종목 중 BBW를 만족하는 최적 타점 종목 발굴합니다.")
...
with tab_korea_etf:
    ...
    st.markdown("##### 🎯 [전략 2] ETF BB상단 돌파(익일 종가) + 모멘텀 상위 25% + BBW D1~D10 🎯")

    # 데이터프레임으로 깔끔하게 표출
    ref_data = {
        "BBW 분위수 (수치범위)": ["D1(1분위) (0.001~0.060)", "D2(2분위) (0.060~0.077)", "D3(3분위) (0.077~0.094)", "D4(4분위) (0.094~0.114)", "D5(5분위) (0.114~0.133)", "D6(6분위) (0.133~0.158)", "D7(7분위) (0.158~0.190)", "D8(8분위) (0.190~0.236)", "D9(9분위) (0.236~0.324)", "D10(10분위) (0.324~1.070)"],
        "1M 승률(수익)": ["58.9% ( 0.01%)", "56.8% ( 0.51%)", "57.4% ( 0.76%)", "64.1% ( 2.07%)", "64.6% ( 4.04%)", "66.4% ( 3.50%)", "62.3% ( 2.67%)", "63.6% ( 3.37%)", "65.1% ( 3.73%)", "62.0% ( 6.64%)"],
        "3M 승률(수익)": ["62.2% ( 0.30%)", "59.8% ( 1.78%)", "62.5% ( 2.68%)", "57.3% ( 1.82%)", "55.8% ( 2.50%)", "58.2% ( 2.85%)", "59.0% ( 4.67%)", "59.2% ( 5.64%)", "66.3% ( 7.15%)", "39.5% ( 9.39%)"],
        "6M 승률(수익)": ["66.2% ( 3.06%)", "56.4% ( 4.28%)", "61.4% ( 5.86%)", "54.4% ( 5.61%)", "53.8% ( 7.14%)", "50.0% ( 4.87%)", "53.1% ( 7.64%)", "49.5% ( 7.39%)", "62.0% (15.55%)", "27.2% (11.41%)"],
        "12M 승률(수익)": ["65.3% ( 8.17%)", "60.5% ( 8.78%)", "61.8% (12.51%)", "62.0% (15.87%)", "56.0% (16.37%)", "60.1% (15.71%)", "53.4% (17.42%)", "41.5% (15.30%)", "41.9% (21.86%)", "17.2% (25.01%)"],
        "발생횟수": ["574회", "574회", "573회", "574회", "573회", "574회", "573회", "574회", "573회", "574회"]
    }
        ...
        with st.spinner("한국 상장 ETF 전종목을 병렬로 스크래핑 및 분석 중입니다..."):
            ...
            if not results:
                ...
                if valid_mom.empty:
                    ...
                    # 2. 돌파 여부 확인 (당일 또는 전일 돌파) - MA50 제외, BB상단만 남김
